...
@pytest.fixture
def token():
    token = OVLToken.deploy({'from': accounts[0]})
    token.grantRole(token.MINTER_ROLE(), accounts[1].address, {
        'from': accounts[0]
    })
    token.grantRole(token.BURNER_ROLE(), accounts[1].address, {
        'from': accounts[0]
    })
    return token
...
def test_mint(token):
    token.mint(accounts[1], 100*BASE, {'from': accounts[1]})
...
def test_burn(token):
    ...
    token.burn(accounts[1], 60*BASE, {'from': accounts[1]})
